...
Now includes support for generating non-overlapping spans.
"""
...
class GenerationConfig:
    ...
    non_overlapping: bool = False  # Whether to generate non-overlapping spans
...
class CharacterSequenceGenerator:
    ...
    def _generate_random_pattern(self, min_length: int, max_length: int) -> str:
        ...
        pattern = ""
        wildcard_used = False  # Track if we've already used a wildcard

        for _ in range(length):
            # In non-overlapping mode, allow at most one wildcard per pattern
            can_use_wildcard = not self.config.non_overlapping or not wildcard_used

            if can_use_wildcard and random.random() < self.config.wildcard_probability:
                pattern += "."
                wildcard_used = True
            else:
    ...
    def _check_overlap(self, pos1: Tuple[int, int], pos2: Tuple[int, int]) -> bool:
    ...
    def inject_patterns_batch(
        ...
        used_positions = set()
        injected_spans = []  # Track all injected pattern spans
        ...
        for query_idx, query in enumerate(queries):
            ...
            for pattern_num in range(num_patterns):
                ...
                # If non-overlapping mode is enabled, find a non-overlapping position
                if self.config.non_overlapping:
                    attempts = 0
                    while (
                        attempts < 50
                    ):  # Try up to 50 times to find non-overlapping position
                        proposed_span = (base_pos, base_pos + pattern_length)
                        overlaps = any(
                            self._check_overlap(proposed_span, existing)
                            for existing in injected_spans
                        )
                        if not overlaps:
                            break
                        # Try a different position
                        base_pos = random.randint(
                            1, max(1, sequence_length - pattern_length - 2)
                        )
                        attempts += 1

                    if attempts >= 50:
                        # Skip this pattern if we can't find a non-overlapping position
                        position_offset += 1
                        continue
                ...
                # Record the injected span
                injected_spans.append((base_pos, base_pos + pattern_length))
...
class WordSequenceGenerator:
    ...
    def inject_patterns_batch(
        ...
        # Track used positions to prevent overlaps
        used_positions = set()
        injected_spans = []  # Track all injected pattern spans (in word indices)
...
class SyntheticDatasetGenerator:
    ...
    def _verify_non_overlapping(self, matches: List[Tuple[int, int]]) -> bool:
        """Verify that all matches are non-overlapping"""
        for i in range(len(matches)):
            for j in range(i + 1, len(matches)):
                if not (
                    matches[i][1] <= matches[j][0] or matches[j][1] <= matches[i][0]
                ):
                    return False
        return True
    ...
    def generate_example(self) -> Example:
        ...
        for query in queries:
            ...
            # If non-overlapping mode, verify matches don't overlap
            if self.config.non_overlapping and not self._verify_non_overlapping(
                matches
            ):
                # Filter to keep only non-overlapping matches (greedy approach)
                non_overlapping_matches = []
                sorted_matches = sorted(matches, key=lambda x: x[0])
                for match in sorted_matches:
                    if not any(
                        self.sequence_generator._check_overlap(match, existing)
...
def main():
    ...
    # NEW: Non-overlapping spans example
    print("\n" + "=" * 50)
    print("Generating NON-OVERLAPPING spans dataset...")

    non_overlap_config = GenerationConfig(
        mode=GenerationMode.WORD,
        sequence_length=60,
        num_examples=100,
        language="en",
        ...
        max_pattern_length=2,
        wildcard_probability=0.3,
        non_overlapping=True,  # Enable non-overlapping mode
    )

    non_overlap_generator = SyntheticDatasetGenerator(non_overlap_config)
    non_overlap_dataset = non_overlap_generator.generate_dataset()
    non_overlap_generator.save_dataset(
        non_overlap_dataset, "non_overlapping_english_word_dataset.json"
    )

    # Print example with verification
    example = non_overlap_dataset[0]
    print("\nNon-overlapping Word Example:")
    print(f"Input: {example.input_sequence}")
    for i, (query, answer) in enumerate(zip(example.queries, example.answers)):
        print(f"Query {i + 1}: {query.natural_language}")
        print(f"Matches ({len(answer)}): {answer}")
        # Verify non-overlapping
        if non_overlap_generator._verify_non_overlapping(answer):
            print("  ✓ All matches are non-overlapping")
        else:
            print("  ✗ WARNING: Overlapping matches found!")
